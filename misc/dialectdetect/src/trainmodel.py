...
def get_wav(language_num):
    ...
    '''
    y, sr = librosa.load('../audio/{}.wav'.format(language_num.Print()))
...
if __name__ == '__main__':
    ...
    # X_train = K.map_fn(get_wav, X_train)
    # X_test = K.map_fn(get_wav, X_test)
    # X_train, X_test = np.array(X_train), np.array(X_test)
    # Convert to MFCC
    if DEBUG:
        print('converting to mfcc')
    # X_train = K.map_fn(to_mfcc, X_train)
    # X_test = K.map_fn(to_mfcc, X_test)
